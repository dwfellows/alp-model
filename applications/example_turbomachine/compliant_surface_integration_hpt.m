...
% Driver function to compute stability of turbomachine configuration

close all;
clear all;
clc;

% Import fluid and structural utilities
...
fluid_data = dlmread('./computation_data/fluid_solution_data/hp_wall_mesh_data.csv');
...
elem_node = importdata('./computation_data/structural_solution_data/hp_elems.txt'); elem_node = elem_node.data;
node_elem = dlmread('./computation_data/structural_solution_data/hp_node_elem.txt');
nodes = dlmread('./computation_data/structural_solution_data/hp_nodes.txt');
structural_node_locs = nodes(:,2:4);  
surface_node_data = dlmread('./computation_data/structural_solution_data/hp_face_nodes.txt');
surface_node_locs = surface_node_data(:,2:4);
struct_faces = dlmread('./computation_data/structural_solution_data/hp_faces.txt');


%% Align coordinate systems
disp('Aligning coordinate systems')

% Define structural reference points -- in order origin, vec1_pt, vec2_pt
structural_mesh_info = dlmread('./computation_data/structural_solution_data/mesh_ref_pts_hpt.csv');
...
fluid_mesh_info = dlmread('./computation_data/fluid_solution_data/mesh_ref_pts_hpt.csv');
...
import_mat_props = dlmread('./computation_data/structural_solution_data/mat_props_hpt.csv');
...
% Define mode data
full_mode_data = readtable('./computation_data/run_data_hpt.dat');
run_priority = input('Which run priority (1-9/11/13-14/17-19) is to be analyzed?');
mode_type = ' ';
while (mode_type == ' ')
  if ((run_priority < 1) | (run_priority == 10) | (run_priority == 12) | (run_priority == 15) | (run_priority == 16) | (run_priority > 25))
